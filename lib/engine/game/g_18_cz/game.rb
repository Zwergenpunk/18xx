  ...
  module Game
    module G18CZ
      class Game < Game::Base
        ...
        attr_accessor :rusted_variants, :vraclav, :vraclavs_corporations
        ...
        def setup
          ...
          @rusted_variants = []
          @vraclavs_corporations = []

          unless multiplayer?
            @vraclav = Player.new(-1, 'Vraclav')
          ...
          new_corporation_for_vraclave(:small) unless multiplayer?

          block_lay_for_purple_tiles
          init_player_debts
        end

        def new_corporation_for_vraclave(size)
          ...
          new_corporation.owner = @vraclav
          @recently_floated << new_corporation
          @vraclavs_corporations << new_corporation

          par_value = PAR_RANGE[new_corporation.type][0]
          ...
          until new_corporation.floated?
            ...
            @share_pool.transfer_shares(bundle, @vraclav)
            index += 1
          end

          new_train_for_vraclav(new_corporation)
        end

        def new_train_for_vraclav(corporation)
        ...
        def active_players
          ...
          active.reject { |item| item == @vraclav }
        end
        ...
        def multiplayer?
          @multiplayer ||= @players.reject { |item| item == @vraclav }.size >= 3
        end
        ...
        def operating_round(round_num)
          unless multiplayer?
            track_lay_player = @players.reject { |item| item == @vraclav }[1]
            @vraclavs_corporations.each { |item| item.owner = track_lay_player }
          end
        ...
        def event_medium_corps_available!
          ...
          new_corporation_for_vraclave(:medium) unless multiplayer?
        end
        ...
        def event_large_corps_available!
          ...
          new_corporation_for_vraclave(:large) unless multiplayer?
        end
        ...
        def or_set_finished
          if multiplayer?
            ...
            @vraclavs_corporations.each do |item|
              item.owner = @vraclav
              new_train_for_vraclav(item)
            end
          end
        end
